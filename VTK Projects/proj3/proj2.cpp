...
BoundingBoxForCell(const float *X, const float *Y, const int *dims,
                   int cellId, float *bbox);//Similar to source in 330
float
...
//void GetLogicalPointIndex(int *idx, int pointId, const int *dims)
	...
	//Y*|X|+X just getting a pixel in a image
	scalerFieldValues[0] = F[cellCord[1] * dims[0] + cellCord[0]];
	scalerFieldValues[1] = F[cellCord[1] * dims[0] + cellCord[0]+1];
	scalerFieldValues[2] = F[(cellCord[1]+1) * dims[0] + cellCord[0]];
	scalerFieldValues[3] = F[(cellCord[1]+1) * dims[0] + cellCord[0]+1];
...
//                     bbox[1]: the maxXmum X value in cellId.
//                     bbox[2]: the minimum Y value in cellId.
//                     bbox[3]: the maxXmum Y value in cellId.
//
...
{
    int totalSize = (dims[0]-1)*(dims[1]-1);
    if(totalSize <= cellId)
    {
	bbox[0] = -100;
	bbox[1] = 100;
    ...
    {
	int xy[2];		
	GetLogicalCellIndex(xy,cellId,dims); 
	bbox[0] = X[xy[0]];
	bbox[1] = X[xy[0]+1];
	bbox[2] = Y[xy[1]];
	bbox[3] = Y[xy[1]+1];
...
for(j = 0; j < maxY-1;j++)
	{
		...
		{	
			Fbox[0] = F[j*maxX+i];
			Fbox[1] = F[(j*maxX+i)+1];
			Fbox[2] = F[(j*maxX+i)+maxX];
			Fbox[3] = F[(j*maxX+i)+maxX+1];
			bool oneNeg = false;
			bool onePos = false;
			int k;
			for(k = 0; k < 4; k++)
