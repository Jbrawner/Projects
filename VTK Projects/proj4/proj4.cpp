                              ...
                              const float *Y, const float *F, float *rv)
{
	...
	float xScalerValues[4];
	//|X|*Y+X just getting a pixel in a image
	xScalerValues[0] = F[(dims[0] * cellCord[1] + cellCord[0])*2];
	xScalerValues[1] = F[(dims[0] * cellCord[1] + cellCord[0]+1)*2];
	xScalerValues[2] = F[(dims[0] * (cellCord[1]+1) + cellCord[0])*2];
	xScalerValues[3] = F[(dims[0] * (cellCord[1]+1) + cellCord[0]+1)*2];

	float t1,t2; //t1 for 1 & 2
	float FA,FB,FX;
	t1 = (xPoint - X[cellCord[0]])/(X[cellCord[0]+1] - X[cellCord[0]]);     
	FA = xScalerValues[0] + (t1*(xScalerValues[1] - xScalerValues[0])); 
	FB = xScalerValues[2] + (t1*(xScalerValues[3] - xScalerValues[2])); 
	t2 = (yPoint - Y[cellCord[1]])/(Y[cellCord[1]+1] - Y[cellCord[1]]);
	FX = FA + (t2*(FB-FA)); 

	 rv[0] = FX; // setting the x-component of the velocity
	float yScalerValues[4];
	//|X|*Y+X just getting a pixel in a image
	yScalerValues[0] = F[(dims[0] * cellCord[1] + cellCord[0])*2+1];
	yScalerValues[1] = F[(dims[0] * cellCord[1] + cellCord[0]+1)*2+1];
	yScalerValues[2] = F[(dims[0] * (cellCord[1]+1) + cellCord[0])*2+1];
	yScalerValues[3] = F[(dims[0] * (cellCord[1]+1) + cellCord[0]+1)*2+1];

	t1 = (xPoint - X[cellCord[0]])/(X[cellCord[0]+1] - X[cellCord[0]]);     
	FA = yScalerValues[0] + (t1*(yScalerValues[1] - yScalerValues[0])); 
	FB = yScalerValues[2] + (t1*(yScalerValues[3] - yScalerValues[2])); 
	t2 = (yPoint - Y[cellCord[1]])/(Y[cellCord[1]+1] - Y[cellCord[1]]);
	FX = FA + (t2*(FB-FA)); 

	rv[1] = FX; // setting the y-component of the velocity
	}    
	    //My Code
	//end of else
}
...
{
    float fencePostXY[2];
    EvaluateVectorFieldAtLocation(pt,dims,X,Y,F,fencePostXY);
    float firstLocationVal = pt[0]; // pt[0]+h*fencePostXY[0];
    float secondLocationVal = pt[1]; // pt[1]+h*fencePostXY[1];
    output_locations[0] = firstLocationVal; // set the x component of the first output location
    output_locations[1] = secondLocationVal; // set the y component of the first output location
    speeds[0] = sqrt(pow(fencePostXY[0],2) + pow(fencePostXY[1],2)); 
...
//End of fencepost
	...
	float xyVel[2];
	float inputXY[2];
	inputXY[0] = output_locations[i*2];
	inputXY[1] = output_locations[i*2+1];
	EvaluateVectorFieldAtLocation(inputXY,dims,X,Y,F,xyVel);
	float xValSquared = pow(xyVel[0],2);
	float yValSquared = pow(xyVel[1],2);
	speeds[i+1] = sqrt(xValSquared + yValSquared);
	float xOffset = (float)h*xyVel[0];
	float yOffset = (float)h*xyVel[1];
	output_locations[i*2+2] = inputXY[0]+ xOffset;
	output_locations[i*2+3] = inputXY[1]+ yOffset; 	
    }
}
// ****************************************************************************
// ****************************************************************************
void
RK4(const float *pt, const int *dims, const float *X, 
                    const float *Y, const float *F, 
                    float h, int nsteps, float *output_locations, float *speeds)
{
    /*
float fencePostXY[2];
    ...
    speeds[0] = sqrt(pow(fencePostXY[0],2) + pow(fencePostXY[1],2)); 
*/
float fencePostk1[2], fencePostk2[2], fencePostk3[2], fencePostk4[2], k4Output[2];
float pt2[2], pt3[2], pt4[2];
    EvaluateVectorFieldAtLocation(pt,dims,X,Y,F,fencePostk1);
    pt2[0] = pt[0] + (h/2) * fencePostk1[0];
    pt2[1] = pt[1] + (h/2) * fencePostk1[1];
    EvaluateVectorFieldAtLocation(pt2,dims,X,Y,F,fencePostk2);
    pt3[0] = pt[0] + (h/2) * fencePostk2[0];
    pt3[1] = pt[1] + (h/2) * fencePostk2[1];
    EvaluateVectorFieldAtLocation(pt3,dims,X,Y,F,fencePostk3);
    pt4[0] = pt[0] + h * fencePostk3[0];
    pt4[1] = pt[1] + h * fencePostk3[1];
    EvaluateVectorFieldAtLocation(pt4,dims,X,Y,F,fencePostk4);
    k4Output[0] = pt[0] + (1/6) * h *(fencePostk1 + (2* fe))
...
CalculateArcLength(const float *output_locations, int nlocations)
{
    ...
    { 				      
	pt1X = output_locations[i*2];  
	pt1Y = output_locations[i*2+1];
	pt2X = output_locations[i*2+2];
	pt2Y = output_locations[i*2+3];
	float x = pow(pt1X - pt2X,2);
	float y = pow(pt1Y - pt2Y,2);
	total += sqrt(x+y);
    }
    return total;
}
